"""Options Analytics Page with MongoDB Integration"""

import reflex as rx
from ..state import OptionsState

def options_analytics_page() -> rx.Component:
    """Main options analytics page with data display"""
    return rx.vstack(
        # Header
        rx.hstack(
        ...
        # Metrics Cards
        rx.grid(
            metric_card("Total Contracts", OptionsState.total_contracts.to_string()),
            metric_card("Avg IV", OptionsState.avg_iv_pct),
            metric_card("Max OI", OptionsState.max_oi.to_string()),
